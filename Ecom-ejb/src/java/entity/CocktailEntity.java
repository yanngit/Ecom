...
@NamedQueries({
    ...
    @NamedQuery(name = "getCocktailsByExpName", query = "SELECT c FROM CocktailEntity c WHERE LOWER(c.name) LIKE :exp"),
    @NamedQuery(name = "getCocktailsByExpRecipe", query = "SELECT c FROM CocktailEntity c WHERE LOWER(c.recipe) LIKE :exp"),
